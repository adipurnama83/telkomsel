...
def log(value, status='Telkomnyet', color='[CC]'):
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('AKU TAK SANGGUP MELIHAT ENGKAU BAHAGIA DENGAN DIA:( '.format(self.proxy_host, self.proxy_port))
            ...
            self.log('KINI KURELA KAN ENGKAU BAHAGIA DENGAN DIA, WALAU DIRIKU TERLUKA   ''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
    	 ...
    	 '[G1]''Fery Anggoro','[CC]'
    	 '=============================================','[CC]'
     'Remod   : Fery Anggoro','[CC]'
     'Script  : Telkomsel Opok No Malak Kuota Flash','[CC]'
     'Limit   : Silahkan Test Sendiri','[CC]'
     'Facebook: Fery Anggoro','[CC]'
     'Penting : Tkp Jateng Wajib Pakai Apn Aha','[CC]'
    ])))    
